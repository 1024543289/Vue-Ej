...
export default {
  ...
  actions: {
    ...
    async deleteCustomerById({ dispatch }, id) {
      // 1. 删除顾客信息
      const response = await get('http://134.175.154.93:6677/customer/deleteById?id=' + id)
      // 2. 刷新
    ...
    async saveOrUpdateCustomer({ dispatch, commit }, customer) {
      // 1. 提交请求
      const response = await post('http://134.175.154.93:6677/customer/saveOrUpdate', customer)
      // 2. 关闭模态
      commit('closeModal')
      // 3. 刷新页面
      dispatch('findAllCustomers')
